...
export const serveImageController = (req: Request, res: Response, next: NextFunction) => {
  ...
  return res.sendFile(path.resolve(DIR_UPLOAD_IMAGE, name), (err) => {
    if (err) {
...
export const serveM3u8Controller = (req: Request, res: Response, next: NextFunction) => {
  ...
  return res.sendFile(path.resolve(DIR_UPLOAD_VIDEO, realId, 'master.m3u8'), (err) => {
    if (err) {
      res.status((err as any).status).send('Not Found')
    }
  })
...
export const serveSegmentController = (req: Request, res: Response, next: NextFunction) => {
  ...
  return res.sendFile(path.resolve(DIR_UPLOAD_VIDEO, id, v, segment), (err) => {
    if (err) {
      res.status((err as any).status).send('Not Found')
    }
  })
...
export const serveVideoStreamController = (req: Request, res: Response, next: NextFunction) => {
  ...
  const { name } = req.params
  const videoPath = path.resolve(DIR_UPLOAD_VIDEO, name)
